...
func main() {
    ...
    flag.BoolVar(&verbose, "v", true, "Enable debug output")
...
`, os.Args[0], dir, base, base, base)
    ...
    raft.SetLogLevel(0)
    raft.RegisterCommand(&raft.ExecuteCommand{})
    raft.RegisterCommand(&raft.ExecuteCommandReply{})
    ...
    // Start the server
    go func() {
        s, err := server.New(directory, listen, join)
        if err != nil {
            log.Fatal(err)
        }
